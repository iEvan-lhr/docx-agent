...
type Footer struct {
	...
	RelativePath string     // 文件在 zip 包中的路径，如 word/Footer1.xml
	ID           string     // relationship ID
	Attrs        []xml.Attr // <--- 用于存储根元素的属性
}

// NewFooter creates a new Footer instance
func NewFooter(root *RootDoc) *Footer {
	return &Footer{
		root: root,
	}
}
...
func (h Footer) MarshalXML(e *xml.Encoder, start xml.StartElement) (err error) {
...
func (h *Footer) UnmarshalXML(d *xml.Decoder, start xml.StartElement) (err error) {
...
// LoadFooterXml loads Footer from XML bytes
func LoadFooterXml(rd *RootDoc, fileName string, fileBytes []byte) (*Footer, error) {
	Footer := NewFooter(rd)
	err := xml.Unmarshal(fileBytes, Footer)
	if err != nil {
		return nil, err
	}
	Footer.RelativePath = fileName
	return Footer, nil
}
